...
class ProductApi {
  ...
  Future<http.Response> addProduct(Product product) {
    final uri = Uri.https(BASE_URL, PRODUCTS_END_POINT, {'auth': token});
    return http.post(uri, body: product.encode());
  }

  Future<Product> updateProduct(Product product) async {
    final uri =
        Uri.https(BASE_URL, "/products/${product.id}.json", {'auth': token});
  ...
  Future<void> deleteProduct(productId) async {
    final uri =
        Uri.https(BASE_URL, "/products/$productId.json", {'auth': token});
  ...
  Future<bool> toggleFavorite(String productId, bool isFavorite) async {
    final uri = Uri.https(BASE_URL, "/products/$productId.json", {'auth': token});
    final response =
